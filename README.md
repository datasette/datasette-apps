...
If [datasette-agent](https://github.com/datasette/datasette-agent) is installed, this plugin registers tools that let the agent build and edit stored apps: `app_list`, `app_create`, `app_view`, `app_str_replace`, `app_insert`, `app_edit`, `app_render`, `app_set_stored_queries`, `app_set_csp_origins` and `app_debug`. These respect the same `create-app`, `edit-app` and `apps-set-csp` permissions as the web interface.

Allowing an app to call stored queries or contact external origins is a data access grant, so the agent cannot do it on its own. When `app_create` is called with `stored_queries` or `csp_origins`, when `app_set_stored_queries` adds a query an app did not already have, or when `app_set_csp_origins` adds a new origin, the agent pauses and asks you to approve the grant in the chat. For stored queries the question shows each query's name, title, description, parameters, whether it is a write query and its full SQL. For origins it lists each new origin and explains that the app could send data it can read to them. Both questions also list any queries or origins that would lose access. Approving creates the app or records a new app revision; declining leaves things unchanged and tells the agent to check with you. Queries that do not exist or that you cannot view, and origins outside the allow-list for users without `apps-set-csp`, are rejected before you are asked. Removing queries or origins from an app does not require approval.
